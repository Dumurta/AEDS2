...
typedef struct
{
    int id;
    char name[256];
    char releaseDate[16]; /* dd/mm/aaaa */
    int estimatedOwners;
    float price;
    char supportedLanguages[1024];
    int metacriticScore; /* -1 se vazio */
    float userScore;     /* -1.0 se vazio/tbd */
...
static int less_by_name_then_id(const Game *a, const Game *b, Metrics *m)
{
    int r = 0;
    int c = strcmp(a->name, b->name);
    m->comparacoes = m->comparacoes + 1; /* comparação por nome */
    if (c < 0)
        r = 1;
    else if (c > 0)
        r = 0;
...
static void selection_sort_by_name(Game *v, int n, Metrics *m)
{
    int i = 0;
    m->comparacoes = 0;
    m->movimentacoes = 0;
    ...
    while (i < n - 1)
    {
        ...
        while (j < n)
        {
            if (less_by_name_then_id(&v[j], &v[min], m))
                min = j;
            j = j + 1;
        }
        if (min != i)
            swap_game(&v[i], &v[min], m);
        i = i + 1;
    }
}
    ...
    if (s != NULL)
    {
        ...
        while (i >= 0 && r == -1)
        {
            if (s[i] >= '0' && s[i] <= '9')
            {
                ...
                if (val > 2147483647L)
                    val = 2147483647L; /* clamp para int */
                r = (int)val;
            }
            i = i - 1;
        }
...
int main(int argc, char **argv)
{
    ...
    Game *arr = NULL; /* todos os jogos carregados do CSV */
    int arr_cap = 0, arr_len = 0;

    Game *sel = NULL; /* jogos selecionados pelos IDs de entrada */
    ...
    if (have_error == 0)
    {
        ...
        t0 = clock();
        selection_sort_by_name(sel, sel_len, &mt);
        t1 = clock();
...

        {
            ...
            FILE *lf = fopen("884985_selecao.txt", "wb");
            if (lf != NULL)
            {
